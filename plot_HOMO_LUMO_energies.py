...
font_size = 16
def store_files_in_list(filename):
    try:
...
if calculation_is_spin_polarized:
    ...
    for line in file[eigenvalues_index + 5: eigenvalues_index + 5 + no_of_KS_states]:
        ...
        if (up_LUMO_index == None) and (up_occupation < 0.3):
    ...
    for line in file[eigenvalues_index + 5 + no_of_KS_states + 4: eigenvalues_index + 5 + 2*no_of_KS_states + 4]:
        ...
        if (dn_LUMO_index == None) and (dn_occupation < 0.3):
            dn_LUMO_index = dn_orbital[0]
            dn_HOMO_index = dn_LUMO_index - 1

    # 5. Create a list containing the orbitals to be plotted, print to terminal, and create a dictionary containing the degenerate orbitals according to energy_tolerance criterion
    ...
    for line in file[eigenvalues_index + 5 + up_HOMO_index - up_no_OMOs - 1: eigenvalues_index + 5 + up_LUMO_index + up_no_UMOs]:
        print(line[:-1])
    print('  Spin-down eigenvalues')
    for line in file[eigenvalues_index + 5 + no_of_KS_states + 4 + dn_HOMO_index - dn_no_OMOs - 1: eigenvalues_index + 5 + no_of_KS_states + 4 + dn_LUMO_index + dn_no_UMOs]:
        print(line[:-1])

    print(f'\nEnergy tolerance in your calculation is ({energy_tolerance_decimal_string} eV).')
    rounding_criteria = save_no_of_orbitals('Enter how many digits after the decimal point you want to consider for the energy (in eV): ')
    ...
    ax.plot([-up_max_degeneracy*101, -up_max_degeneracy*100], [max_E_Ha, max_E_Ha], color=LUMO_color, linewidth=2, label='Unoccupied orbitals')
    ax.plot([-up_max_degeneracy*101, -up_max_degeneracy*100], [min_E_Ha, min_E_Ha], color=HOMO_color, linewidth=2, label='Occupied orbitals')
    ...
    for rounded_energy in up_orbitals_degeneracy:
        ...
        for index in indexes_in_same_energy:
            ...
            ax.plot([x_position - bar_width, x_position + bar_width], [plot_energy, plot_energy], color=orbital_color, linewidth=line_width)
...
################### Non-spin-polarized calculation ##################
    ...
    for line in file[eigenvalues_index + 3 : eigenvalues_index + 3 + no_of_KS_states]:
        ...
        if (LUMO_index == None) and (occupation < 0.6):
    ...
    ax.plot([-max_degeneracy*1.1, -max_degeneracy], [max_E_Ha, max_E_Ha], color=LUMO_color, linewidth=2, label='Unoccupied orbitals')
    ax.plot([-max_degeneracy*1.1, -max_degeneracy], [min_E_Ha, min_E_Ha], color=HOMO_color, linewidth=2, label='Occupied orbitals')
