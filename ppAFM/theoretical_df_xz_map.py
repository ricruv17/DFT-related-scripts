...
- Parses df.xsf volumetric data into a structured pandas DataFrame containing
  spatial coordinates (x, y, z) and LCPD values.
...
def parse_xsf_volumetric(filename):
    ...
    proportionality_constant = 1000 #from V to mV
    coords = []
...
def interactive_xy_plane_scroll(df_table, initial_z=None, n_colors=10):
    ...
    title_xy = ax_xy.set_title(f"LCPD at Z ≈ {current_z:.4f} Å")
    cbar = fig.colorbar(img, ax=ax_xy, label="LCPD (mV)")
    
    # Z-line subplot
    ax_z.set_xlabel("Z (Å)")
    ax_z.set_ylabel("LCPD (mV)")
    ...
    def update_xy_plot(z_index):
        ...
        if not df_zslice.empty:
            ...
            img.set_clim(vmin=-70, vmax=320)
            title_xy.set_text(f"LCPD at Z ≈ {current_z:.4f} Å")
...
def interactive_xz_plane_scroll(df_table, initial_y=None, n_colors=10):
    z_values = np.sort(df_table['y'].unique())
    z_index = 0
    if initial_y is not None:
        z_index = (np.abs(z_values - initial_y)).argmin()
    current_z = z_values[z_index]

    df_zslice = df_table[np.isclose(df_table['y'], current_z)].copy()
    pivot_table = df_zslice.pivot(index='z', columns='x', values='frequency')
    pivot_table = pivot_table.sort_index(ascending=True).sort_index(axis=1, ascending=True)
    ...
    ax_xy.set_xlabel("X (Å)")
    ax_xy.set_ylabel("Z (Å)")
    title_xy = ax_xy.set_title(f"LCPD at Y ≈ {current_z:.4f} Å")
    cbar = fig.colorbar(img, ax=ax_xy, label="LCPD (mV)")
    ...
    def on_click(event):
        ...
        # Plot Z-line on the right subplot
        ax_z.plot(z_line['y'], z_line['frequency'], color=color, label=f"(x={x_found:.4f}, z={y_found:.4f})")
        ax_z.legend()
        fig.canvas.draw_idle()
...
if not xsf_files:
    raise FileNotFoundError("LCPD_volts.xsf not found in the current folder.")
...
interactive_xz_plane_scroll(df_table, initial_y=20.46592, n_colors=10)
